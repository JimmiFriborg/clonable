      ...
      <Card>
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <CardTitle className="mt-2">Visible progress across the MVP</CardTitle>
          </div>
          <div className="rounded-full border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700">
            Completed work stays motivating
          </div>
        </div>

        <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {dashboard.featureProgress.map((feature) => (
            <div key={feature.id} className="rounded-[24px] border border-slate-200 bg-white p-4">
              <div className="flex items-center justify-between gap-3">
                <h3 className="font-display text-lg font-semibold tracking-tight text-slate-950">
                  {feature.title}
                </h3>
                <span className="text-sm font-medium text-slate-500">{feature.progressPercent}%</span>
              </div>
              <p className="mt-2 text-sm text-slate-500">{feature.status}</p>
              <div className="mt-4">
